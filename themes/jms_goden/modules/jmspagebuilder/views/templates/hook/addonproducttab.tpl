...
<div class="jms-tab">
	...
	{$cf = 0}
		{if $config.show_new eq '1'}
			<li class="nav-item"><a class="button active button" data-toggle="tab" href="#latest">{l s='New Arrivals'  d='Shop.Theme'}</a></li>
			{$cf = $cf + 1}
		{/if}
		...
		{if $config.show_featured eq '1'}
			<li class="nav-item"><a class="{if $cf eq 0}active{/if} button" data-toggle="tab" href="#featured">{l s='Featured'  d='Shop.Theme'}</a></li>
		{$cf = $cf + 1}
		{/if}			
...
<div class="view-all">
	<a href="{$urls.base_url}index.php?id_category=10&controller=category&id_lang=1">Shop All Items</a>
</div>
